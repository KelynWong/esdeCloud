...
exports.processDesignSubmission = async function (req, res) {
    ...
    let { designTitle, designDescription, file ,file_type} = req.body
    /* let file = req.body.file; */
    /* check if any input is missing */
    if (designTitle == null || designDescription == null||file==null||file_type==null) {
        res.status(400).json({ message: "messing input" });
        return;
    }
    if(!REGEXP_MINE_TYPE.test(file_type)){
        res.status(415).json({message:"Invalid file type"});
        return;
    }
    /* check it is file */
    if (REGEXP_TEXT.test(designTitle) && REGEXP_TEXT.test(designDescription) && (!REGEXP_FILE_NAME.test(file))) {
        try {
            file = "images/"+Date.now().toString() + "_" + file;
            ...
            let result = await fileDataManager.createFileData(file, userId, designTitle, designDescription)
            ...
            let data = await fileDataManager.s3.createPresignedPost( {
                Bucket: "esde2",
                ContentType:file_type,
                Expires:300,
                Fields:{key:file,acl: "private"},
                Conditions:[{acl: "private"},["content-length-range", 0, 1048576]]
            })
...
exports.processGetSubmissionData = async function (req, res) {
    ...
    try {
        ...
        if (results) {
            /*output sanitization */
            results[0].forEach(x => {
                x.file_url = fileDataManager.s3.getSignedUrl('getObject', {
                    Bucket: "esde2",
                    Key: x.file_url,
                    Expires: 60 * 5
                })
...
exports.processGetOneDesignData = async function (req, res) {
    ...
    try {
        ...
        if (result) {
            ...
            result.file_url = fileDataManager.s3.getSignedUrl('getObject', {
                Bucket: "esde2",
                Key: result.file_url,
                Expires: 60 * 5
            })

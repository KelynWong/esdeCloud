 ...
 */
module.exports.createFileData = (imageURL, userId, designTitle, designDescription) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                let query = `INSERT INTO file ( file_url , 
                 design_title, design_description,created_by_id ) 
                 VALUES (?,?,?,?) `;
                connection.execute(query, [ imageURL, designTitle, designDescription, userId], (err, rows) => {
                    if (err) {
...
module.exports.getFileData = (userId, pageNumber, search) => {
    ...
    if ((search == '') || (search == null)) {
        ...
        /*fixed*/
        designFileDataQuery = `SELECT file_id,file_url,design_title,design_description 
